...
- **`sts`** — one STS `AssumeRole` per vend with an inline session policy
  ...
  **covered by automated tests against MinIO; the real-AWS path has been run on
  a cloud deployment by the maintainer but is not yet in the automated suite**.
...
- Verified end to end against MinIO (sign → execute → 200; sibling-table
  ...
  support). **Automated against MinIO; run on real AWS by the maintainer, not
...
- **Foreign assets are read-only** (conflict-free federation): a foreign table
  ...
  `register`, `drop`, `rename`, or `reportMetrics` targeting a foreign table —
  or any write under a `mirror__<name>` warehouse — is rejected with a `409
...
- **Commit-invariant preservation**: the hook adds no new pointer-mutation path
  ...
  property suite (`commit_properties`, `commit_properties_pg`) passes
