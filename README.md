...
- **Enforce row/column policy inside the scan plan.** Row filters are compiled into the residual every planning client must apply to each row, and an ABAC deny aborts planning with a 403 before a single file is returned — the same mechanism the walled gardens reserve for themselves, from a neutral catalog any engine can point at. Column masks strip the column's statistics from the plan so its values and ranges never leak through plan metadata; full byte-level column prevention is a layered guarantee, stated honestly per engine and path in [the enforcement matrix](docs/design/enforcement-matrix.md).
- **Repair what engines emit.** Flink sprays small files; some engines write pathological layouts. Meridian's compaction rewrites them into house style through a normal, audited, revertible commit — every row preserved (verified by reading the rewritten Parquet back through real Iceberg manifests and comparing every row). Engines' weaknesses become the catalog's feature.
...
- **Federation** — mirror external Iceberg REST catalogs as read-only foreign assets, with a sprawl dashboard across every catalog you own. (AWS Glue is a planned source type; today only `iceberg-rest` sources sync — a `glue` mirror can be registered but not yet synced.)
- **Autonomous table maintenance** — a health model and a built-in compaction engine that rewrites small files into large ones through the normal commit path (audited and revertible), snapshot expiry, and a savings ledger. Compaction preserves every row — verified in the executor test suite by reading the rewritten Parquet back through real Iceberg manifests and comparing a sorted projection row-for-row, with the maintenance worker confirming the commit lands and reduces the live file count.
- **Cross-engine access governance** — row filters, column masks, and Cedar ABAC defined once and applied inside server-side scan planning: row filters ride along as residuals every planning client must apply, ABAC denies abort planning with a 403, and column masks withhold the column's statistics from the plan. What each engine and access path *prevents* versus *detects* is stated plainly in the [enforcement matrix](docs/design/enforcement-matrix.md) — no uniform-enforcement overclaim.
