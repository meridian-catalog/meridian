...
| `test_catalog_as_code.py` | `meridian` CLI | n/a | catalog-as-code: `meridian apply -f` a bundle (warehouse, two-level namespace, role, grant, webhook with a `${ENV}` secret), verify every resource through the management/IRC APIs, re-apply and assert a zero-change no-op, `plan` and assert no drift, then delete a role out-of-band and assert `plan` reports the drift and re-`apply` reconciles it; also asserts an unset `${ENV}` secret fails closed. Builds the debug `meridian` binary if not already present |
