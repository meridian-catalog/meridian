...
| Spark | 3.5.6 (iceberg-spark-runtime 1.11.0) | ✅ namespace + partitioned table DDL | ✅ batched `INSERT`s | ✅ add column, old rows read back as `NULL` | ✅ scans, aggregates, `VERSION AS OF` time travel | ✅ merge-on-read `MERGE INTO` + `DELETE FROM`; position-delete files and snapshot operations verified over REST | ✅ `CREATE VIEW` + `SELECT` — the create required a Meridian fix, and `CREATE OR REPLACE VIEW` of an existing view still fails, see note ‡ |
...
same fresh-id treatment to `createView`. The remaining gap:
`CREATE OR REPLACE VIEW` of an *existing* view goes through `replaceView`,
whose `add-schema` update still validates ids strictly, and fails with the
same error. Details and reproduction:
[`spark/README.md`](spark/README.md#known-gap-create-or-replace-view) and
